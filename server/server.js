...
function requestHandler(request, response) {
    ...
    let requestedResourse = path.join(
    ...
    // use the exists method of the fs module to check if the requestedResource exists
    fs.exists(requestedResourse, function(exists) {
        ...
        // check if the requested resourse is a directory. If it is, just set our index.html page as the requested resource
        if (fs.statSync(requestedResourse).isDirectory()) {
            requestedResourse += '/index.html';
        }

        // read the requested file (asynchronously) and send it's content to the client
        fs.readFile(
            requestedResourse, // path to the requested resource
            "binary", // read the requested resource as a binary file
            function(err, file) { // call back function to handle end of file reading

                // if an error occured while reading the file, send the error message with a status code of 500 (Internal server error)
                if (err) {
                ...
                //helper object to map requested content types (extension) to response mime types
                ...
                const contentType = contentTypesByExtension[
                    path.extname(requestedResourse)
                ];

                // if the requested resource maps to any of our content type extension, then set the Content-Type field for our response headers
                if (contentType) {
...
// create an instance of our httpServer and pass in our request handler callback
